...
Ukurasa huu unagusia njia zifuatazo za kuondoa kujirudia:

1. Kutambua na kuondoa safu zinazojirudia
2. "Kuvunjavunja" safu na kubarikisha baadhi ya safu (mfano: ndogo kuliko zote au kubwa kuliko zote) kutoka katika kundi la safu
3. Kuungisha thamani kutoka katika safu nyingi na kuunda safu moja

<!-- ======================================================= -->
## Mandalizi { }


### Pakia Vifurushi {.unnumbered}

This code chunk shows the loading of packages required for the analyses. In this handbook we emphasize `p_load()` from **pacman**, which installs the package if necessary *and* loads it for use. You can also load installed packages with  `library()` from **base** R. See the page on [R basics] for more information on R packages.  


Sehemu hii ya msimbo inaonyesha upakiaji wa vifurushi vinavyohitajika kwa uchanganuzi. Katika kitabu hiki cha mwongozo tunasisitiza matumizi ya kikotozi cha `p_load()` kutoka kifushi kiitwacho **pacman**, ambacho husakinisha kifurushi kikihitajika wakati bado hakipo kwenye RStudio *na* kukipakia kwa matumizi. Unaweza pia kupakia vifurushi vilivyosakinishwa kwa kutumia kifurushi cha `library()` kutoka R ya **msingi**. Tazama ukurasa kwenye [Misingi ya R] kwa maelezo zaidi kuhusu vifurushi vya R.
...
### Kuingiza data {.unnumbered}

For demonstration, we will use an example dataset that is created with the R code below.

Kwa onyesho, tutatumia mfano wa limbiko data ambalo limeundwa kwa msimbo wa R hapa chini.
...
Data tutakazotumia kwa mfano ni rekodi za matukio ya UVIKO-19 kwenye simu, zinajumuisha waliokutana na wagonjwa na waliopata UVIKO-19. Safu wima ni pamoja na `recordID` (kilichotengenezwa kwa kompyuta), `personID`, `name`, `date` ya kukutana, `time` wa kukutana, `purpose` la mkutano (ama kuhoji kama kesi au kama mtu anayewasiliana naye. ), na `symptoms_ever` (kama mtu katika tukio hilo aliripoti *kuwahi* kuwa na dalili).

Here is the code to create the `obs` dataset:  

Hapa kanuni ya R ya kuunda limbikodata ya `obs`:
...
#### Hapa kuna sura ya data {#dedup_data .unnumbered} 

Use the filter boxes along the top to review the encounters for each person.  

Tumia visanduku vya vichungi vilivyo juu ili kukagua mikutano ya kila mtu.
...
Mambo machache ya kuzingatia unapokagua data:

* Rekodi mbili za kwanza ni nakala kamili 100% ikijumuisha nakala `recordID` (lazima iwe hitilafu ya kompyuta!)
* Safu mlalo mbili za pili ni nakala, katika safu wima zote *isipokuwa `recordID`*
* Watu kadhaa walikuwa na mawasiliano mengi ya simu, kwa tarehe na nyakati tofauti, na kama anwani na/au visa
* Katika kila kukutana, mtu huyo aliulizwa ikiwa  **aliwahi** kuwa na dalili, na baadhi ya habari hizi hazipo.


And here is a quick summary of the people and the purposes of their encounters, using `tabyl()` from **janitor**:

Na hapa kuna mukhtasari wa haraka wa watu na makusudio ya kukutana kwao, kwa kutumia `tabyl()` kutoka kwa **janitor**:
...
Sehemu hii inaeleza jinsi ya kukagua na kuondoa nakala za safu mlalo katika fremu ya data. Inaonyesha pia jinsi ya kushughulikia vitu vinavyojirudia kwenye vekta.
